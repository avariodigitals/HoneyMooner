...
class HM_Destination_Meta {
    ...
    public static function render(WP_Post $post): void {
        ...
        <div class="hm-grid hm-grid-2">
            <p><label><strong>Country</strong></label><input type="text" class="widefat" name="hm_destination[country]" value="<?php echo esc_attr($meta['country']); ?>"></p>
    ...
    public static function save(int $post_id): void {
        ...
        foreach (['country','continent','image','subtitle','intro_content','best_time_to_visit','seo_title','meta_description'] as $field) {
            update_post_meta($post_id, $field, sanitize_textarea_field(wp_unslash($data[$field] ?? '')));
        }
        update_post_meta($post_id, 'starting_price', is_numeric($data['starting_price'] ?? null) ? (float) $data['starting_price'] : 0);
